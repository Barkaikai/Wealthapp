...
export class BriefingAggregator {
  ...
  async aggregateBriefingData(userId: string): Promise<BriefingData> {
    ...
    const [
      ...
      this.storage.getCrmContacts(userId).catch(() => []),
      this.storage.getCrmOrganizations(userId).catch(() => []),
      this.storage.getCrmLeads(userId).catch(() => []),
      this.storage.getCrmActivities(userId).catch(() => []),
