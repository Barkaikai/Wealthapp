...
import { fetchUpcomingGoogleCalendarEvents, GoogleCalendarNotConnectedError, GoogleCalendarScopeError } from "./googleCalendar";
...
// Local dev: placeholder key; payments disabled without real key
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  if (process.env.CSRF_SECRET && process.env.LOCAL_DEV_AUTH !== '1') {
  ...
  } else if (process.env.LOCAL_DEV_AUTH === '1') {

    console.log('[CSRF] Disabled for local development auth');
  ...
  app.get('/api/memory', isAuthenticated, async (req: any, res) => {
    ...
    try {
      ...
      const memories = await storage.getMemories(userId, limit);
      ...
      res.status(500).json({ message: error.message || 'Failed to fetch memories' });
  ...
  app.post('/api/memory', isAuthenticated, async (req: any, res) => {
    ...
    try {
      ...
      const { kind, title, content, tags = [] } = req.body;

      if (!kind || !title || !content) {

        return res.status(400).json({ message: 'kind, title, and content are required' });

      }

      const memory = await storage.createMemory({ userId, kind, title, content, tags });
  ...
  app.post('/api/briefing/generate', aiRateLimiter, isAuthenticated, async (req: any, res) => {
    ...
    try {
      ...
      await storage.createMemory({
        ...
        content: JSON.stringify({ highlights, risks, actions }),

        tags: ['briefing', 'daily', 'ai'],

      }).catch((memoryError) => {

        console.warn('Memory save skipped:', memoryError?.message || memoryError);
  ...
  app.post('/api/calendar/google/sync', isAuthenticated, async (req: any, res) => {

    try {

      const userId = await getCanonicalUserId(req.user.claims);

      const requestedDays = Number(req.body?.days ?? req.query.days ?? 90);

      const daysAhead = Number.isFinite(requestedDays) ? Math.min(Math.max(Math.trunc(requestedDays), 1), 365) : 90;

      const googleEvents = await fetchUpcomingGoogleCalendarEvents(daysAhead);
      ...
      for (const googleEvent of googleEvents) {
        ...
        try {
          ...
          const existingEvent = existingByGoogleId.get(parsed.googleEventId);

          if (existingEvent) {

            const updatedEvent = await storage.updateCalendarEvent(existingEvent.id, userId, parsed);

            existingByGoogleId.set(parsed.googleEventId, updatedEvent);
            ...
            const createdEvent = await storage.createCalendarEvent(parsed);

            existingByGoogleId.set(parsed.googleEventId, createdEvent);
          ...
          console.warn('Skipped Google Calendar event during sync:', eventError);
...
${processedText}`;
  ...
  // Chat route (Ollama first, OpenAI fallback when configured)
  app.post('/api/chat', isAuthenticated, async (req, res) => {
    try {
      ...
      const message = String(error?.message || 'Failed to get chat response');
      if (message.includes('Ollama') || message.includes('fetch failed')) {
        return res.status(503).json({ message: 'Local AI service is unavailable. Make sure Ollama is running.' });
      }
